    ...
    if (propsGl != null) {
        ...
        if (re === 'USE-GLOBAL') {
            ...
            if (!doesFileExist(GLOBALPropertyFileName)) {
                ...
                /* TODO: Use the question synch function
                ...
                const result = run('npx tcli ask-question -a "USE-GLOBAL is set for property ' + col.blue(propName) + ', but no global configuration is set. Do you want to configure this now ?:YES:NO"', false)
                if (result) {
                    log(INFO, 'Setting global config')
                    run('tcli -g')
                } else {
                    log(INFO, 'Ok no global config will be set')
                }
            }
