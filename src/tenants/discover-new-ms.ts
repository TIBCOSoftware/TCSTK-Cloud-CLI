...
export async function uploadDataSetFile () {
  ...
  if (typeForUpload.toLowerCase() !== 'none') {
    ...
    const endpoint = replaceEndpoint(CCOM.clURI.dis_file_upload)
...
export async function importDiscoverDataset () {
  log(INFO, 'Importing Discover Dataset 2...')
  prepDiscoverProps()
...
export async function uploadDiscoverAsset () {
  ...
  if (fileToUpload && fileToUpload !== '' && doesFileExist(fileToUpload)) {
    ...
    // }
    const endpoint = replaceEndpoint(CCOM.clURI.dis_nms_assets)
